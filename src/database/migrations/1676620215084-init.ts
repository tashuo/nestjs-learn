...
export class init1676620215084 implements MigrationInterface {
    name = 'init1676620215084'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`post\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`content\` varchar(255) NOT NULL, \`content_type\` enum ('html', 'markdown', 'text') NOT NULL DEFAULT 'text', \`status\` enum ('normal', 'delete') NOT NULL DEFAULT 'normal', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`userId\` int NOT NULL, INDEX \`IDX_5c1cf55c308037b5aca1038a13\` (\`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`post_tags_tag\` (\`postId\` int NOT NULL, \`tagId\` varchar(36) NOT NULL, INDEX \`IDX_b651178cc41334544a7a9601c4\` (\`postId\`), INDEX \`IDX_41e7626b9cc03c5c65812ae55e\` (\`tagId\`), PRIMARY KEY (\`postId\`, \`tagId\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`tag\` ADD CONSTRAINT \`FK_d0dc39ff83e384b4a097f47d3f5\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
