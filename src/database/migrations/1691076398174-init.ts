...
export class Init1691076398174 implements MigrationInterface {
    name = 'Init1691076398174'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`collect_posts\` (\`id\` int NOT NULL AUTO_INCREMENT, \`remark\` varchar(255) NOT NULL COMMENT '备注', \`createdAt\` datetime(6) NOT NULL COMMENT '创建时间' DEFAULT CURRENT_TIMESTAMP(6), \`collectId\` int NULL, \`postId\` int NULL, UNIQUE INDEX \`uniq_collect_post\` (\`collectId\`, \`postId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`comment_likes\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL COMMENT '创建时间' DEFAULT CURRENT_TIMESTAMP(6), \`userId\` int NULL, \`commentId\` int NULL, UNIQUE INDEX \`uniq_user_comment\` (\`userId\`, \`commentId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`post_likes\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL COMMENT '创建时间' DEFAULT CURRENT_TIMESTAMP(6), \`userId\` int NULL, \`postId\` int NULL, UNIQUE INDEX \`uniq_user_post\` (\`userId\`, \`postId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`notices\` (\`id\` int NOT NULL AUTO_INCREMENT, \`type\` enum ('follow', 'like', 'collect', 'comment') NOT NULL COMMENT '类型', \`content\` varchar(1000) NOT NULL COMMENT '内容', \`is_read\` tinyint NOT NULL COMMENT '是否已读', \`created_at\` datetime(6) NOT NULL COMMENT '创建日期' DEFAULT CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`userId\` int NULL, \`operatorId\` int NULL, \`postId\` int NULL, \`commentId\` int NULL, INDEX \`idx_user_type_created\` (\`userId\`, \`type\`, \`created_at\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user_entity_followers_user_entity\` (\`userEntityId_1\` int NOT NULL, \`userEntityId_2\` int NOT NULL, INDEX \`IDX_0cfac94a7190e05681bd30031b\` (\`userEntityId_1\`), INDEX \`IDX_f218c630db489c465aa55e55ae\` (\`userEntityId_2\`), PRIMARY KEY (\`userEntityId_1\`, \`userEntityId_2\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`category_posts_post_entity\` (\`categoryId\` int NOT NULL, \`postEntityId\` int NOT NULL, INDEX \`IDX_8c9c125a5f88531cc63059f898\` (\`categoryId\`), INDEX \`IDX_7b80ef4fbfb55d12ddb367a0c2\` (\`postEntityId\`), PRIMARY KEY (\`categoryId\`, \`postEntityId\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`post_entity_tags_tag\` (\`postEntityId\` int NOT NULL, \`tagId\` varchar(36) NOT NULL, INDEX \`IDX_30d145f2cb22c758e78c4ba74e\` (\`postEntityId\`), INDEX \`IDX_89178b840f4e5a3e7481f3e58d\` (\`tagId\`), PRIMARY KEY (\`postEntityId\`, \`tagId\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`comments\` ADD CONSTRAINT \`FK_e44ddaaa6d058cb4092f83ad61f\` FOREIGN KEY (\`postId\`) REFERENCES \`post_entity\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`comments\` ADD CONSTRAINT \`FK_7e8d7c49f218ebb14314fdb3749\` FOREIGN KEY (\`userId\`) REFERENCES \`user_entity\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        ...
        await queryRunner.query(`ALTER TABLE \`collect_posts\` ADD CONSTRAINT \`FK_6a5fd455e5473286061747998de\` FOREIGN KEY (\`postId\`) REFERENCES \`post_entity\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`comment_likes\` ADD CONSTRAINT \`FK_34d1f902a8a527dbc2502f87c88\` FOREIGN KEY (\`userId\`) REFERENCES \`user_entity\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`notices\` ADD CONSTRAINT \`FK_79364067097eea7912bb08855b6\` FOREIGN KEY (\`userId\`) REFERENCES \`user_entity\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`notices\` ADD CONSTRAINT \`FK_33cc34d83dafa017a42e977f3f3\` FOREIGN KEY (\`operatorId\`) REFERENCES \`user_entity\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`notices\` ADD CONSTRAINT \`FK_0f6eddabf51a7a71abf0befc2c8\` FOREIGN KEY (\`postId\`) REFERENCES \`post_entity\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`notices\` ADD CONSTRAINT \`FK_2fee6b75e702c6aa393110dfafc\` FOREIGN KEY (\`commentId\`) REFERENCES \`comments\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`notices\` DROP FOREIGN KEY \`FK_2fee6b75e702c6aa393110dfafc\``);
        await queryRunner.query(`ALTER TABLE \`notices\` DROP FOREIGN KEY \`FK_0f6eddabf51a7a71abf0befc2c8\``);
        await queryRunner.query(`ALTER TABLE \`notices\` DROP FOREIGN KEY \`FK_33cc34d83dafa017a42e977f3f3\``);
        await queryRunner.query(`ALTER TABLE \`notices\` DROP FOREIGN KEY \`FK_79364067097eea7912bb08855b6\``);
        ...
        await queryRunner.query(`DROP INDEX \`IDX_89178b840f4e5a3e7481f3e58d\` ON \`post_entity_tags_tag\``);
        await queryRunner.query(`DROP INDEX \`IDX_30d145f2cb22c758e78c4ba74e\` ON \`post_entity_tags_tag\``);
        await queryRunner.query(`DROP TABLE \`post_entity_tags_tag\``);
        await queryRunner.query(`DROP INDEX \`IDX_7b80ef4fbfb55d12ddb367a0c2\` ON \`category_posts_post_entity\``);
        await queryRunner.query(`DROP INDEX \`IDX_8c9c125a5f88531cc63059f898\` ON \`category_posts_post_entity\``);
        await queryRunner.query(`DROP TABLE \`category_posts_post_entity\``);
        await queryRunner.query(`DROP INDEX \`IDX_f218c630db489c465aa55e55ae\` ON \`user_entity_followers_user_entity\``);
        await queryRunner.query(`DROP INDEX \`IDX_0cfac94a7190e05681bd30031b\` ON \`user_entity_followers_user_entity\``);
