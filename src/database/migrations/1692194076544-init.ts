...
export class Init1692194076544 implements MigrationInterface {
    name = 'Init1692194076544';

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE TABLE \`admin_users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`username\` varchar(255) NOT NULL COMMENT '用户名', \`nickname\` varchar(255) NOT NULL COMMENT '昵称' DEFAULT '', \`password\` varchar(255) NOT NULL COMMENT '密码', UNIQUE INDEX \`IDX_2873882c38e8c07d98cb64f962\` (\`username\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
            `CREATE TABLE \`admin_permissions\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL COMMENT '名称', \`slug\` varchar(255) NOT NULL COMMENT '唯一标识', \`http_method\` varchar(255) NOT NULL COMMENT 'http方法', \`http_path\` varchar(10000) NOT NULL COMMENT 'http路径', \`weight\` int NOT NULL COMMENT '权重' DEFAULT '0', \`parentId\` int NULL, UNIQUE INDEX \`IDX_8f7ddce91a5867342143ffe7d1\` (\`slug\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
            `CREATE TABLE \`admin_roles\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL COMMENT '名称', \`slug\` varchar(255) NOT NULL COMMENT '唯一标识', UNIQUE INDEX \`IDX_f5e394f493b5d8593abbb9dcb9\` (\`slug\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
            `CREATE TABLE \`admin_role_users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`userId\` int NULL, \`roleId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
            `CREATE TABLE \`admin_role_permissions\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`permissionId\` int NULL, \`roleId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
            `CREATE TABLE \`admin_role_menus\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`menuId\` int NULL, \`roleId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
            `CREATE TABLE \`admin_menus\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL COMMENT '名称', \`uri\` varchar(255) NOT NULL COMMENT 'uri', \`icon\` varchar(255) NOT NULL COMMENT 'icon', \`show\` tinyint NOT NULL COMMENT '是否展示' DEFAULT 0, \`weight\` int NOT NULL COMMENT '权重' DEFAULT '0', \`parentId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE \`post_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL DEFAULT '', \`content\` varchar(10000) NOT NULL DEFAULT '', \`image_paths\` varchar(255) NOT NULL COMMENT '图片路径,多个逗号分隔' DEFAULT '', \`content_type\` enum ('html', 'markdown', 'text') NOT NULL DEFAULT 'text', \`status\` enum ('normal', 'delete') NOT NULL DEFAULT 'normal', \`comment_count\` int NOT NULL COMMENT '评论数' DEFAULT '0', \`like_count\` int NOT NULL COMMENT '点赞数' DEFAULT '0', \`repost_count\` int NOT NULL COMMENT '转发数' DEFAULT '0', \`collect_count\` int NOT NULL COMMENT '收藏数' DEFAULT '0', \`detail_count\` int NOT NULL COMMENT '详情页浏览数' DEFAULT '0', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`userId\` int NOT NULL, INDEX \`IDX_5e32998d7ac08f573cde04fbfa\` (\`userId\`), INDEX \`IDX_96fcac0a31e5cb86357aae7f0d\` (\`comment_count\`), INDEX \`IDX_14a22293494ffe29a7de6af5bd\` (\`like_count\`), INDEX \`IDX_74b998ec1edaac0b4b318fef4b\` (\`detail_count\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE \`comments\` (\`id\` int NOT NULL AUTO_INCREMENT, \`content\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`like_count\` int NOT NULL COMMENT '点赞数' DEFAULT '0', \`reply_count\` int NOT NULL COMMENT '回复数量，只有根节点存储该值' DEFAULT '0', \`mpath\` varchar(255) NULL DEFAULT '', \`postId\` int NOT NULL, \`userId\` int NOT NULL, \`parentId\` int NULL, INDEX \`IDX_e44ddaaa6d058cb4092f83ad61\` (\`postId\`), INDEX \`IDX_7e8d7c49f218ebb14314fdb374\` (\`userId\`), INDEX \`IDX_45fc7b7794b2a5a6de50648a79\` (\`like_count\`), INDEX \`IDX_02a459982cd42bf377068322b3\` (\`reply_count\`), INDEX \`idx_mpath\` (\`mpath\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE \`notices\` (\`id\` int NOT NULL AUTO_INCREMENT, \`type\` enum ('follow', 'like', 'collect', 'comment') NOT NULL COMMENT '类型', \`content\` varchar(1000) NOT NULL COMMENT '内容' DEFAULT '', \`is_read\` tinyint NOT NULL COMMENT '是否已读' DEFAULT 0, \`created_at\` datetime(6) NOT NULL COMMENT '创建日期' DEFAULT CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`userId\` int NULL, \`operatorId\` int NULL, \`postId\` int NULL, \`commentId\` int NULL, INDEX \`idx_user_type_created\` (\`userId\`, \`type\`, \`created_at\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`admin_permissions\` ADD CONSTRAINT \`FK_2103721c8dfcd14bcb36011a7c8\` FOREIGN KEY (\`parentId\`) REFERENCES \`admin_permissions\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`admin_menus\` ADD CONSTRAINT \`FK_73921a2edea234b490130ec34ba\` FOREIGN KEY (\`parentId\`) REFERENCES \`admin_menus\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
        );
        await queryRunner.query(
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`admin_menus\` DROP FOREIGN KEY \`FK_73921a2edea234b490130ec34ba\``,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`admin_permissions\` DROP FOREIGN KEY \`FK_2103721c8dfcd14bcb36011a7c8\``,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(`DROP TABLE \`admin_role_menus\``);
        await queryRunner.query(`DROP TABLE \`admin_role_permissions\``);
        await queryRunner.query(`DROP TABLE \`admin_role_users\``);
        await queryRunner.query(`DROP INDEX \`IDX_f5e394f493b5d8593abbb9dcb9\` ON \`admin_roles\``);
        await queryRunner.query(`DROP TABLE \`admin_roles\``);
        await queryRunner.query(
            `DROP INDEX \`IDX_8f7ddce91a5867342143ffe7d1\` ON \`admin_permissions\``,
        );
